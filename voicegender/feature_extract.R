...
if (length(setdiff(packages, rownames(installed.packages()))) > 0) {
  install.packages(setdiff(packages, rownames(installed.packages())), repos = 'https://mirrors.tuna.tsinghua.edu.cn/CRAN/')  
}
library(tuneR)
library(seewave)
library(caTools)
...
processFolder <- function(fileName) {
  ...
  acoustics$duration <- NULL
  acoustics$sound.files <- NULL
  acoustics$selec <- NULL
  acoustics$peakf <- NULL

  acoustics <- acoustics[complete.cases(data),]
...
# Load data
#data <- processFolder('arctic_a0001.wav')
#print(data)
